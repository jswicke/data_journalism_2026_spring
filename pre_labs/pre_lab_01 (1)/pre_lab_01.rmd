...
**Task** In the console (look below), type 2+2 and hit enter. What does it return? Type your answer below. **4**
...
**7314 observations, 29 variables**
...
In the space below, write the datatype for the "EIN" column and the "ASSET_AMT" column (it's in light gray, offset by \< \>). **EIN: character, ASSET_AMT: doubles**
...
In the space below, write the name of the church from the first row of the "name" column. **Healing and deliverance ministry inc**
...
**group_by tells R what variable to pull from, summarise looks for each instance of the variable, and count shows the number**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **It arranges the data to place the highest numbers first**
...
**The output showed two results using the group_by line**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **The sum command was used, which now uses all the church size data from a denomination. The top terms are either protestants or don't follow a specific denomination**
...
1. Which Maryland churches make the most revenue?

2. Are any of the largest megachurches in Maryland?

3. What state has the most megachurches
