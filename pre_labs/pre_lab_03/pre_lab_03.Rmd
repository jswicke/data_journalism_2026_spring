...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **It does not**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Vendor name is a character, amount is a double, date is a character.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **8,484 zip codes do not have five characters. These could either be typos or it could be the area's zip+4 code.**
...
**Task** Run the following codeblock to filter the payments data to show only payments to vendors with "CHURCH" in their name. How many payments were made to churches? What was the total amount paid to churches? **There were 1,095 total payments with $177,497,135 in total payments.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **There are several rows that aren't categorized as either a grant or loan.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Many grantors have duplicates (or very similar names)**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **rename is self-explanatory, but mutate creates a new column with all the names being changed to uppercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **58 duplicates are possible**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **21,197**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **There is now a row of exclusively 5-digit zip codes**
